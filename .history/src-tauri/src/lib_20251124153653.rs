...
pub fn run() {
    ...
    Builder::default()
        .manage(app_state)
        .setup(move |app| {
            // 获取默认窗口并设置 initialization_script
            if let Some(window) = app.get_webview_window("main") {
                // 先关闭
                window.close().ok();
            }
            
            // 创建新窗口并设置 initialization_script（使用不同标签）
            let window = WebviewWindowBuilder::new(app, "main_proxy", tauri::WebviewUrl::External("http://test-otc.68chat.co/".parse().unwrap()))
                .title("Backstage68")
